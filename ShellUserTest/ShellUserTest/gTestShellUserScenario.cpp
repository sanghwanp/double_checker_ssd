...
class TestShell {
 public:
  virtual std::string Exec(const std::vector<std::string>& commands) {
    ...
    RemoveTestOutput("ssd_nand.txt");
    RemoveTestOutput("ssd_output.txt");
    return oss.str();
  }

 private:
  void RemoveTestOutput(std::string fileName) {
    struct stat buffer;

    if (stat(fileName.c_str(), &buffer)) return;
...
class TestShellFixture : public Test {
 public:
  ...
  void SendCommandToMockForTestScript(const std::vector<std::string>& command) {
    EXPECT_CALL(mockShell, Exec(_))
        .WillOnce(Return(TEST_SCRIPT_PASS_OUTPUT));
    std::string result = mockShell.Exec(command);
    EXPECT_THAT(
        result,
        AllOf(HasSubstr(TEST_SCRIPT_PASS_OUTPUT),
